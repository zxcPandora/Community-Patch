...
//	var browseNetworkButton
	...
	int MainCommunityID
	int TransferCommunityID = 276928 //Transfer station
} file
...
{
	print("[PATCH TEST]Before Leave the Community ID: " + file.communityInfoPanelWidgets.CommunityId )
	if( AreWeInCommunity( file.communityInfoPanelWidgets.CommunityId ) )
	{
		if( file.communityInfoPanelWidgets.CommunityId == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == true && file.communityInfoPanelWidgets.CommunityId == file.TransferCommunityID )
		{
		...
		else if( file.communityInfoPanelWidgets.CommunityId == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == true && file.communityInfoPanelWidgets.CommunityId != file.TransferCommunityID )
		{
			LeaveCommunity( file.communityInfoPanelWidgets.CommunityId )
			SetActiveCommunity( file.TransferCommunityID )
		}
		else if( file.communityInfoPanelWidgets.CommunityId == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == false )
		{
			LeaveCommunity( file.communityInfoPanelWidgets.CommunityId )
			JoinCommunity( file.TransferCommunityID )
			SetActiveCommunity( file.TransferCommunityID )
		}
		else if( file.communityInfoPanelWidgets.CommunityId != file.MainCommunityID )
		{
			LeaveCommunity( file.communityInfoPanelWidgets.CommunityId )
			SetActiveCommunity( file.MainCommunityID )
		}
		file.MainCommunityID = file.TransferCommunityID
	}
}
...
{
	print("[PATCH TEST]Before Leave the Current Community ID: " + GetCurrentCommunityId() )
	if( AreWeInCommunity( GetCurrentCommunityId() ) )
	{
		if( GetCurrentCommunityId() == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == true && GetCurrentCommunityId() == file.TransferCommunityID )
		{
			//WARNING:DANGEROUS BEHAVIOR!!!
			//WARNING:DANGEROUS BEHAVIOR!!!
			//WARNING:DANGEROUS BEHAVIOR!!!
		...
		else if( GetCurrentCommunityId() == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == true && GetCurrentCommunityId() != file.TransferCommunityID)
		{
			LeaveCommunity( GetCurrentCommunityId() )
			SetActiveCommunity( file.TransferCommunityID )
		}
		else if( GetCurrentCommunityId() == file.MainCommunityID && AreWeInCommunity( file.TransferCommunityID ) == false )
		{
			LeaveCommunity( GetCurrentCommunityId() )
			JoinCommunity( file.TransferCommunityID )
			SetActiveCommunity( file.TransferCommunityID )
		}
		else if( GetCurrentCommunityId() != file.MainCommunityID )
		{
			LeaveCommunity( GetCurrentCommunityId() )
			SetActiveCommunity( file.MainCommunityID )
		}
...
void function OnMyNetworksMenu_Open()
{
	...
	if ( communityId >= 0 ){
		file.MainCommunityID = communityId
		thread GetCommunityInfoThread( communityId )
	}
	HideAllPanels()
	UI_SetPresentationType( ePresentationType.NO_MODELS )
